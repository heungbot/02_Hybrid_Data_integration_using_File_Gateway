...
프로젝트 소개 : 본 프로젝트의 주요 목표는 기업 내부의 다양한 온프레미스 데이터를 효율적으로 S3 버킷으로 이전하여 DR능력을 확보하고, File Gateway 호스트를 다른 서버에 마운트하여 클라우드에서 온프레미스 데이터를 원활하게 활용하는 것입니다. 이를 위해 회사 내의 특정 온프레미스 서버 시나리오를 가정하고 요구 사항을 충족하는 적합한 아키텍처를 구현하는 프로젝트 입니다. 더불어 재사용이 가능하도록 Terraform 모듈로 구성하였습니다.
